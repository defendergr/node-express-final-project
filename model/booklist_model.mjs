import { Book, BookUser, User } from './bookList_seq_pg.mjs'
...
async function loadComments(booktitle,username) {
    try {
        ...
        if (!booktitle)
            throw new Error("Δεν βρέθηκε το βιβλίο "+booktitle)
        const book = await Book.findOne({ where: { title: booktitle }, raw:true })
        const comments = await BookUser.findAll({ where: { BookTitle: booktitle }, raw:true });
        const userComment = await BookUser.findAll({ where: { BookTitle: booktitle, UserName: username }, raw:true });
        
        if (!comments)
            throw new Error("Δεν βρέθηκαν σχόλια για αυτό το βιβλίο")

        return {comments, book, userComment}
    } catch (error) {
        throw error
    }
}

async function addComment(comment, username) {
    try {
        if (!username)
            throw new Error("Πρέπει να δοθεί όνομα χρήστη")
        ...
        const [com, created] = await BookUser.update(
            {comment: comment.comment,},
            { 
            where: { 
                BookTitle:comment.title,
                UserName: username,
            }})
...
export { addUser, login, loadBooks, addBook, deleteBook, loadComments, addComment }